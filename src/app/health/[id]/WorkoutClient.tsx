...
import { 
  ...
  ExternalLink, Calendar, MoreHorizontal, PlayCircle, Settings
} from 'lucide-react';
...
function SortableExerciseCard({
  ...
  const isLogging = loggingExerciseId === ex._id;
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDragging ? "shadow-xl border-primary/20" : "border-border/40 hover:border-border/80",
    ...
    >
      ...
      <div 
        className={cn(
          "flex items-center p-4 gap-3 cursor-pointer",
          isLogging ? "pb-2" : ""
        )}
        onClick={() => !isLogging && setLoggingExerciseId(ex._id)}
      >
        <button
          {...attributes}
          {...listeners}
          className="p-1.5 rounded-lg text-muted-foreground/30 hover:bg-secondary hover:text-foreground transition-colors cursor-grab active:cursor-grabbing touch-none"
        ...
        <div className="flex-1 min-w-0">
          <h3 className={cn("font-medium truncate transition-colors", hasLogs ? "text-emerald-500" : "text-foreground")}>
            {ex.title}
          </h3>
          ...
          {ex.tutorials?.length > 0 && (
           ...
           <div className="relative">
            <button
              onClick={(e) => {
                e.stopPropagation();
                setShowExerciseMenu(showExerciseMenu === ex._id ? null : ex._id);
              }}
            ...
            {showExerciseMenu === ex._id && (
              <>
                <div 
                  ...
                  onClick={(e) => { e.stopPropagation(); setShowExerciseMenu(null); }}
                />
                <div className="absolute right-0 top-full mt-2 bg-popover border border-border rounded-xl shadow-xl z-50 min-w-40 py-1 overflow-hidden animate-in fade-in-0 zoom-in-95 duration-200">
                  <button
                    ...
                    className="w-full px-4 py-2.5 text-sm text-left hover:bg-secondary/50 transition-colors flex items-center gap-2"
                  >
                    <Edit2 size={14} /> Edit
                  </button>
                  <button
                    ...
                    className="w-full px-4 py-2.5 text-sm text-left hover:bg-rose-500/10 text-rose-500 transition-colors flex items-center gap-2"
                  >
           ...
           <button 
             onClick={(e) => { e.stopPropagation(); setLoggingExerciseId(isLogging ? null : ex._id); }}
             className={cn(
      ...
      >
        <div className="overflow-hidden">
          <div className="px-4 pb-4 pt-0 space-y-4">
            ...
            {/* Last Log Summary */}
            {lastSessionSets && isLogging && !hasLogs && (
              <div className="text-xs text-muted-foreground/70 bg-secondary/30 -mx-4 px-4 py-1.5 flex items-center gap-2">
                <History size={12} />
                <span>Last: {
                    lastSessionSets.map((s: any) => `${s.reps}x${s.weight}kg`).join(', ')
                }</span>
              </div>
            )}
            ...
            {hasLogs && (
               <div className="space-y-2 mt-2">
                 {ex.todaysLog.sets.map((set: Set, idx: number) => (
                    <div 
                      key={set._id || idx} 
                      className="group flex items-center gap-3 text-sm p-2 rounded-lg hover:bg-secondary/50 transition-colors border border-transparent hover:border-border/50"
                    >
                      <div className="w-6 h-6 rounded-full bg-emerald-500/10 text-emerald-500 flex items-center justify-center text-xs font-medium">
                        {idx + 1}
                      </div>
                      ...
                      {editingSetId === set._id ? (
                         ...
                         <div className="flex-1 flex gap-2 items-center">
                            ...
                            <button onClick={() => handleUpdateSet(ex.todaysLog._id, set._id!)} className="p-1 text-emerald-500 hover:bg-emerald-500/10 rounded"><Check size={14}/></button>
                            <button onClick={() => setEditingSetId(null)} className="p-1 text-muted-foreground hover:bg-secondary rounded"><X size={14}/></button>
                         </div>
                      ) : (
                         // Display Mode
                        <>
                          <div className="flex-1">
                             <span className="font-semibold">{set.reps}</span> <span className="text-muted-foreground text-xs">{ex.type === 'duration' ? 'sec' : ex.type === 'distance' ? 'm' : 'reps'}</span>
                             <span className="mx-2 text-border">|</span>
                             <span className="font-semibold">{set.weight > 0 ? set.weight : 'BW'}</span> <span className="text-muted-foreground text-xs">{set.weight > 0 ? 'kg' : ''}</span>
                          </div>
                          <div className="flex opacity-0 group-hover:opacity-100 transition-opacity gap-1">
                             ...
                             <button
                                ...
                                className="p-1.5 text-muted-foreground hover:text-rose-500 rounded-md hover:bg-rose-500/10"
                              >
                                <Trash2 size={12} />
                              </button>
                          </div>
            ...
            {isLogging && (
               <form 
                  ...
                  className="mt-3 pt-3 border-t border-border/30 flex gap-2 items-center animate-in slide-in-from-top-2 duration-200"
               >
                 ...
                 <div className="flex-1 grid grid-cols-2 gap-2">
                    <div className="relative">
                      <input 
                         ...
                         className="w-full bg-secondary/30 hover:bg-secondary/50 focus:bg-background border-none rounded-xl px-3 py-2.5 text-sm transition-all focus:ring-1 focus:ring-primary/30"
                      />
                 ...
                 <button 
                   type="submit"
                   className="h-10 w-10 rounded-xl bg-primary text-primary-foreground flex items-center justify-center hover:opacity-90 active:scale-95 transition-all shadow-sm"
                 >
                   <Check size={18} />
                 </button>
               </form>
...
export default function WorkoutClient({ initialData }: WorkoutClientProps) {
  ...
  const totalVolume = exercises.reduce((acc, ex) => {
    return acc + (ex.todaysLog?.sets?.reduce((sAcc, s) => sAcc + (s.weight * s.reps), 0) || 0);
  }, 0);
  ...
  async function handleLogSet(e: React.FormEvent, moveToNext = false) {
    ...
    // Use last weight if not provided
    const weight = logData.weight ? parseFloat(logData.weight) : (
      exercises.find(e => e._id === loggingExerciseId)?.lastLog?.sets?.[0]?.weight || 0
    );
     // If weight is still 0/empty, check recommended
    const finalWeight = weight || (exercises.find(e => e._id === loggingExerciseId)?.recommendedWeight || 0);
    ...
    setExercises(current => current.map(ex => {
      if (ex._id === loggingExerciseId) {
        return {
          ...
          todaysLog: {
            ...ex.todaysLog,
            sets: [...(ex.todaysLog?.sets || []), { weight: finalWeight, reps }]
          }
        };
      }
      return ex;
    ...
    setLogData(prev => ({ ...prev, reps: '' })); // Keep weight, clear reps

    startTransition(async () => {
       try {
         await logExerciseSet(loggingExerciseId, { weight: finalWeight, reps: reps }, initialData.date);
       } catch (error) {
         console.error(error);
  ...
  // Wrappers for simple handlers to match signature
  const handleDeleteExercise = async (id: string) => {
      if(!confirm('Delete exercise?')) return;
      await deleteExercise(id, initialData.page._id);
      router.refresh();
  };
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
      ...
      if (active.id !== over?.id) {
          ...
          setExercises(newOrder); // Optimistic

          const orderedIds = newOrder.map(ex => ex._id);
          await reorderExercises(initialData.page._id, orderedIds);
      }
  };
  ...
  const dayOfWeek = new Date(initialData.date).toLocaleDateString('en-US', { weekday: 'long' });

  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-32 animate-in fade-in-0 duration-500">
      
      {/* Header */}
      <header className="flex h-14 items-center gap-4 sticky top-0 bg-background/80 backdrop-blur-md z-30 px-1 -mx-1">
        <Link href="/health" className="p-2 -ml-2 rounded-full hover:bg-secondary transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className={cn(
                  ...
                  !muscleMapCollapsed ? "bg-primary/10 text-primary" : "hover:bg-secondary text-muted-foreground"
              )}
            >
              <ActivityIcon active={!muscleMapCollapsed} />
            </button>
        </div>
      </header>
      ...
      {/* Stats / Muscle Map */}
      <div className={cn(
          "overflow-hidden transition-all duration-300 ease-in-out bg-card rounded-3xl border border-border/40",
          muscleMapCollapsed ? "max-h-0 border-none opacity-0" : "max-h-[500px] opacity-100"
      )}>
         <div className="p-6">
            <div className="flex justify-center mb-4">
                <MuscleMap highlightedMuscles={allMuscles} className="h-48 w-auto" />
            </div>
            <div className="grid grid-cols-3 gap-2 text-center text-xs text-muted-foreground border-t border-border/40 pt-4">
                <div>
                    <span className="block text-lg font-bold text-foreground">{exercises.length}</span>
                    Exercises
                </div>
                <div>
                    <span className="block text-lg font-bold text-foreground">{totalSets}</span>
                    Total Sets
                </div>
                <div>
                   <span className="block text-lg font-bold text-foreground">{Math.round(totalVolume)}</span>
                   Vol (kg)
                </div>
            </div>
         </div>
      ...
      {/* Add Exercise - Sticky Bottom or Inline */}
      <div className="bg-card rounded-2xl border border-border/50 p-3 shadow-sm">
         <form onSubmit={handleCreateExercise} className="flex gap-2">
            ...
            <button 
              type="submit"
              disabled={!newExercise.title || isLoading}
            ...
            <button 
               ...
               className={cn("p-3 rounded-xl transition-colors", showCreateOptions ? "bg-secondary text-foreground" : "text-muted-foreground hover:bg-secondary/50")}
            >
               <Settings size={20} />
            </button>
         </form>
         ...
         {showCreateOptions && (
            <div className="mt-3 pt-3 border-t border-border/30 grid grid-cols-3 gap-3 animate-in slide-in-from-top-2">
               <div>
                  <label className="text-[10px] text-muted-foreground uppercase tracking-wider font-bold ml-1">Sets</label>
                  <input type="number" placeholder="3" className="w-full bg-secondary/30 rounded-lg px-3 py-2 text-sm mt-1" 
                    value={newExercise.initialSets} onChange={e => setNewExercise({...newExercise, initialSets: e.target.value})}
                  />
               </div>
               <div>
                  <label className="text-[10px] text-muted-foreground uppercase tracking-wider font-bold ml-1">Reps</label>
                  <input type="number" placeholder="10" className="w-full bg-secondary/30 rounded-lg px-3 py-2 text-sm mt-1" 
                    value={newExercise.initialReps} onChange={e => setNewExercise({...newExercise, initialReps: e.target.value})}
                  />
               </div>
               <div>
                  <label className="text-[10px] text-muted-foreground uppercase tracking-wider font-bold ml-1">Weight</label>
                  <input type="number" placeholder="kg" className="w-full bg-secondary/30 rounded-lg px-3 py-2 text-sm mt-1" 
                    value={newExercise.recommendedWeight} onChange={e => setNewExercise({...newExercise, recommendedWeight: e.target.value})}
                  />
               </div>
            </div>
         )}
      ...
      {/* Modals are placed here (Edit, Tutorial) - Reuse similar code from original just cleaned up */}
           ...
           <div className="bg-card w-full max-w-sm rounded-3xl border border-border/50 shadow-2xl overflow-hidden p-6 scale-100 animate-in zoom-in-95">
              ...
              <form onSubmit={handleUpdateExercise} className="space-y-4">
                  <input 
                    ...
                    className="w-full bg-secondary/30 rounded-xl px-4 py-3 text-sm font-medium" 
                    placeholder="Name"
                  />
                  <div className="flex gap-2 p-1 bg-secondary/30 rounded-xl">
                      {['reps', 'duration', 'distance'].map(t => (
                        <button 
                          key={t} type="button"
                          onClick={() => setEditExercise({...editExercise, type: t})}
                          className={cn(
                            "flex-1 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all",
                            editExercise.type === t ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
                          )}
                        >
                          {t}
                        </button>
                  ...
                  <div className="flex flex-wrap gap-1.5 p-3 min-h-[100px] max-h-[150px] overflow-y-auto bg-secondary/10 rounded-xl border border-border/30">
                     {MUSCLE_LIST.map(m => (
                       <button
                         key={m} type="button"
                         onClick={() => toggleEditMuscle(m)}
                         className={cn(
                           "px-2.5 py-1 rounded-md text-[10px] font-bold uppercase transition-colors border",
                           editExercise.targetMuscles.includes(m) 
                ...
                <div className="p-4 overflow-y-auto space-y-4">
                   {tutorialModal.tutorials.map((t, i) => (
                      <div key={i} className="aspect-video bg-black rounded-xl overflow-hidden relative group">
                          {t.url.includes('youtu') ? (
                             <iframe src={`https://www.youtube.com/embed/${getYouTubeId(t.url)}`} className="w-full h-full" allowFullScreen />
                          ) : (
                             <img src={t.url} className="w-full h-full object-cover" alt="Tutorial" />
                          )}
                      </div>
                   ))}
