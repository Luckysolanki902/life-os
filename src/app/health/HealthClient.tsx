...
import { Scale, Dumbbell, Plus, Activity, ChevronRight, Sparkles, Smile, Meh, Frown, ThumbsUp, ThumbsDown, CheckCircle2 } from 'lucide-react';
...
const MOOD_OPTIONS = [
  { value: 'great', label: 'Great', icon: Sparkles, color: 'text-emerald-400', bg: 'bg-emerald-500/20', border: 'border-emerald-500/50' },
  { value: 'good', label: 'Good', icon: ThumbsUp, color: 'text-green-400', bg: 'bg-green-500/20', border: 'border-green-500/50' },
  { value: 'okay', label: 'Okay', icon: Meh, color: 'text-yellow-400', bg: 'bg-yellow-500/20', border: 'border-yellow-500/50' },
  { value: 'low', label: 'Low', icon: Frown, color: 'text-orange-400', bg: 'bg-orange-500/20', border: 'border-orange-500/50' },
  { value: 'bad', label: 'Bad', icon: ThumbsDown, color: 'text-rose-400', bg: 'bg-rose-500/20', border: 'border-rose-500/50' },
] as const;
...
export default function HealthClient({ initialData }: HealthClientProps) {
  ...
  const [selectedMood, setSelectedMood] = useState<string | null>(mood?.mood || null);
  ...
  async function handleMoodSelect(moodValue: string) {
    ...
    await saveMood(date, moodValue as 'great' | 'good' | 'okay' | 'low' | 'bad');
        ...
        <div className="bg-card rounded-2xl border border-border/50 p-4 sm:p-5">
          ...
          {selectedMood && (
            <div className="mt-4 pt-4 border-t border-border/30 flex items-center justify-center gap-2 text-sm text-muted-foreground">
              ...
              <span>{MOOD_OPTIONS.find(m => m.value === selectedMood)?.label}</span>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="p-4 rounded-xl bg-card border border-border/50">
            <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">Current</p>
            <div className="mt-1 flex items-baseline gap-1">
              <span className="text-xl sm:text-2xl font-bold">{weightStats.current || '-'}</span>
              {weightStats.current > 0 && <span className="text-xs text-muted-foreground">kg</span>}
            </div>
          </div>
          
          <div className="p-4 rounded-xl bg-card border border-border/50">
            <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">BMI</p>
            <div className="mt-1">
              <span className={cn(
                "text-xl sm:text-2xl font-bold",
                !weightStats.bmi ? "text-muted-foreground" :
                Number(weightStats.bmi) < 18.5 ? "text-blue-400" :
                Number(weightStats.bmi) < 25 ? "text-emerald-500" :
                Number(weightStats.bmi) < 30 ? "text-yellow-500" : "text-rose-500"
              )}>
                {weightStats.bmi || '-'}
              </span>
            </div>
            <p className="text-[10px] text-muted-foreground mt-1">
              {!weightStats.bmi ? '' : 
               Number(weightStats.bmi) < 18.5 ? 'Under' :
               Number(weightStats.bmi) < 25 ? 'Healthy' :
               Number(weightStats.bmi) < 30 ? 'Over' : 'Obese'}
            </p>
          </div>

          <div className="p-4 rounded-xl bg-card border border-border/50">
            <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">30d Δ</p>
            <div className="mt-1 flex items-baseline gap-1">
              <span className={cn(
                "text-xl sm:text-2xl font-bold",
                !weightStats.delta ? "text-muted-foreground" :
                Number(weightStats.delta) > 0 ? "text-rose-500" : "text-emerald-500"
              )}>
                {weightStats.delta ? (Number(weightStats.delta) > 0 ? '+' : '') + weightStats.delta : '-'}
              </span>
              {weightStats.delta && <span className="text-xs text-muted-foreground">kg</span>}
            </div>
          </div>
        </div>
      </section>
          ...
          {pages.map((page) => (
            ...
            >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className={cn(
                      ...
                      page.cycleStatus === 'current' ? "text-primary" : "group-hover:text-primary"
                    )}>
                      {page.title}
                    </h3>
                    {page.cycleStatus === 'current' && (
                      <span className="text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-primary text-primary-foreground">Next</span>
                    )}
                  </div>
              ...
              <ChevronRight size={18} className={cn(
                ...
                page.cycleStatus === 'current' ? "text-primary" : "text-muted-foreground group-hover:translate-x-1"
              )} />
            </Link>
          ))}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card w-full max-w-sm p-6 rounded-3xl shadow-xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleCreatePage} className="space-y-4">
              <div>
                <label className="text-xs font-medium text-muted-foreground ml-1">Plan Name</label>
                <input
                  value={pageTitle}
